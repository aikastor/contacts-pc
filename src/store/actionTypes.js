export const CONTACTS_REQUEST = 'CONTACTS_REQUEST';
export const CONTACTS_SUCCESS = 'CONTACTS_SUCCESS ';
export const CONTACTS_FAILURE = 'CONTACTS_FAILURE';

export const ADD_CONTACT_REQUEST = 'ADD_CONTACT_REQUEST';
export const ADD_CONTACT_SUCCESS = 'ADD_CONTACT_SUCCESS';
export const ADD_CONTACT_FAILURE = 'ADD_CONTACT_FAILURE';

export const EDIT_CONTACT_REQUEST = 'ADD_CONTACT_REQUEST';
export const EDIT_CONTACT_SUCCESS = 'ADD_CONTACT_SUCCESS';
export const EDIT_CONTACT_FAILURE = 'ADD_CONTACT_FAILURE';



